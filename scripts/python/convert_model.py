...
def convert_t5x_to_pytorch(variables: dict, *, num_layers: int, num_decoder_layers: int, is_encoder_only: bool):
    ...
    old = traverse_util.flatten_dict(variables)
...
def load_t5x_weights_in_t5(model, config, t5x_checkpoint_path, is_encoder_only):
    ...
    # variables = checkpoints.load_t5x_checkpoint(t5x_checkpoint_path, remap=True, restore_dtype=jnp.float32)
    with open(t5x_checkpoint_path, 'rb') as f:
        variables = from_bytes(frozen_dict.FrozenDict, f.read())
